# fp = r"D:\workspace\Gitdocuments\hw-memory\deprecated_benchmarks\old_benchmark\spec_example\spec2006_470.lbm\lbm_part.asm"
...
from sample.newCFG.cfg import proc_identify
from sample.newCFG.isa import Instruction
from typing import Tuple
from sample.newCFG.read_asm import AsmFileReader, StatementType
from graphviz import Digraph
from sample.newCFG.cfg import draw_proc, find_cycle, has_cycle, proc_draw_edges
...
# segment = segmentReader(r'D:\workspace\Gitdocuments\hw-memory\deprecated_benchmarks\final_benchmark\spec_benchD.asm')
...
# cache analysis
from sample.cache_analysis.new_cache.fixponit import *
import sys
...
for loop in loop_list.values():
    ...
    for tup in page_range:
        ...
        # # loop_bound默认是1的计算结果
        try:
            END.append([tuple(loop_name), int(tup[0] / 4096), float(
                (float(len(mb_must_hit) * loop_bound) + float(len(mb_persistent) * (loop_bound - 1)) + float(0.5 * len(mb_uc) * loop_bound)) / (
                            float(len(mb_persistent) * 1) + float(len(mb_must_miss) * loop_bound) + float(0.5 * len(mb_uc) * loop_bound)))])
            mb_must_hit = list()
            mb_persistent = list()
            mb_must_miss = list()
            mb_uc = list()
